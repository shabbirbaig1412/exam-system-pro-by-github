...
$pass_pct = (float)($statistics['pass_percentage'] ?? 0);
$grades_dist = (array)($statistics['grades'] ?? []);
?>
...
<div class="wrap esp-admin esp-report-page">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:15px;">
        <div>
            ...
            <h1 style="margin:0;">
                <span class="dashicons dashicons-chart-bar" style="font-size:26px;width:26px;height:26px;vertical-align:middle;color:#8c5e00;"></span>
                <?php esc_html_e('Comprehensive Examination Statistics', 'exam-system-pro'); ?>
            </h1>
            <?php if (!empty($exam)): ?>
        ...
        <button type="button" class="button button-primary" onclick="window.print();" style="display:inline-flex;align-items:center;gap:4px;">
            ...
            <?php esc_html_e('Print Statistics', 'exam-system-pro'); ?>
        </button>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(170px, 1fr));gap:14px;margin-bottom:20px;">
        ...
        <div class="esp-card" style="background:#fff;border-radius:8px;padding:16px;border:1px solid #c3c4c7;border-left:4px solid #8c5e00;">
            <span style="font-size:12px;font-weight:600;color:#646970;text-transform:uppercase;"><?php esc_html_e('Pass %', 'exam-system-pro'); ?></span>
        ...
        <div class="esp-card" style="background:#fff;border-radius:8px;padding:16px;border:1px solid #c3c4c7;border-left:4px solid #007074;">
            <span style="font-size:12px;font-weight:600;color:#646970;text-transform:uppercase;"><?php esc_html_e('Highest Marks', 'exam-system-pro'); ?></span>
            <div style="font-size:26px;font-weight:700;color:#007074;margin-top:4px;"><?php echo esc_html((float)($statistics['highest'] ?? 0)); ?></div>
        </div>
        <div class="esp-card" style="background:#fff;border-radius:8px;padding:16px;border:1px solid #c3c4c7;border-left:4px solid #b32d2e;">
            <span style="font-size:12px;font-weight:600;color:#646970;text-transform:uppercase;"><?php esc_html_e('Lowest Marks', 'exam-system-pro'); ?></span>
            <div style="font-size:26px;font-weight:700;color:#b32d2e;margin-top:4px;"><?php echo esc_html((float)($statistics['lowest'] ?? 0)); ?></div>
        </div>
        <div class="esp-card" style="background:#fff;border-radius:8px;padding:16px;border:1px solid #c3c4c7;border-left:4px solid #4f3494;">
            <span style="font-size:12px;font-weight:600;color:#646970;text-transform:uppercase;"><?php esc_html_e('Average Marks', 'exam-system-pro'); ?></span>
    ...
    <!-- Grade Distribution -->
    <?php if (!empty($grades_dist)): ?>
        <div class="esp-card" style="background:#fff;border-radius:8px;padding:20px;border:1px solid #c3c4c7;margin-bottom:20px;">
            <h2 style="font-size:15px;margin:0 0 15px 0;"><?php esc_html_e('Grade Distribution Breakdown', 'exam-system-pro'); ?></h2>
            <div style="display:flex;flex-wrap:wrap;gap:12px;">
                <?php foreach ($grades_dist as $gr => $cnt): 
                    $pct = $total_st > 0 ? round(($cnt / $total_st) * 100, 1) : 0;
                ?>
                    <div style="background:#f6f7f7;padding:12px 18px;border-radius:6px;border:1px solid #dcdcde;min-width:110px;text-align:center;">
                        <span style="font-size:18px;font-weight:700;color:#2271b1;"><?php echo esc_html($gr); ?></span>
                        <div style="font-size:16px;font-weight:600;color:#1d2327;margin-top:2px;"><?php echo esc_html($cnt); ?> <?php esc_html_e('students', 'exam-system-pro'); ?></div>
                        <span style="font-size:12px;color:#646970;"><?php echo esc_html($pct); ?>%</span>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <?php if (!empty($results)): ?>
        <div class="esp-card" style="background:#fff;border-radius:8px;padding:0;overflow:hidden;border:1px solid #c3c4c7;">
            <div style="padding:14px 18px;background:#f6f7f7;border-bottom:1px solid #c3c4c7;">
                <strong style="font-size:14px;color:#1d2327;"><?php esc_html_e('All Candidate Results', 'exam-system-pro'); ?></strong>
            </div>
